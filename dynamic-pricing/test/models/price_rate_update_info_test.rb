...
class PriceRateUpdateInfoTest < ActiveSupport::TestCase
  ...
  test "create persisted" do
    minimal_update_info = PriceRateUpdateInfo.create!(executed_at: Time.current)

    assert minimal_update_info.valid?
    assert minimal_update_info.persisted?

    maximal_update_info = PriceRateUpdateInfo.create!(successful: true, error_message: "", executed_at: Time.current)
